...
import { matchWiki, suggestWiki } from './wiki.ts';
...
const SYSTEM_PROMPT = `你是「TADA醬」（TADA-chan），臺灣科技農企業發展協會（TADA，Taiwan Advanced Agribusiness Development Association）的 AI 秘書小助手。
...
- 近期活動：2026 年 9 月 4 日（星期五）第六屆會員大會暨理事長就職典禮
  - 地點：臻愛婚宴會館 烏日高鐵店（台中市烏日區高鐵路三段 168 號，台中高鐵站步行約 8 分鐘）
  - 內容：會務討論、AI 大師講座、政府補助計畫說明、理事長就職典禮、豐收晚宴
- 新任會長：將於 9/4 會員大會選舉後產生（尚未選出）
...
臺灣科技農企業發展協會（TADA）的 AI 秘書，很開心認識你！
...
// 呼叫 LINE Reply API 回覆訊息（可附 Quick Reply 推薦按鈕）
...
async function handleTextMessage(
  ...
  // ③ 都不會：固定回覆 + 附上「你是不是想問」Quick Reply 推薦 + 記錄 + 通知
  const suggestions = suggestWiki(userText, 4);
  const quickReplyItems: QuickReplyItem[] = suggestions
    .filter((e) => e.suggest)
    .map((e) => ({
      type: 'action',
      action: { type: 'message', label: e.suggest!.label, text: e.suggest!.text },
    }));

  await replyToLine(accessToken, replyToken, FALLBACK_TEXT, quickReplyItems);
  try {
